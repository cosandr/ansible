...
if find --version &>/dev/null; then
    cmd="find files inventory -type f -regextype egrep -regex '.*(\.csr|\.key|vault\.yml|vault\.yml\.j2)$' -print0"
else
    cmd="find -E files inventory -type f -regex '.*(\.csr|\.key|vault\.yml|vault\.yml\.j2)$' -print0"
fi
